...
#if defined(ARDUINO) || defined(__IAR_SYSTEMS_ICC__) || defined(__XC) || defined(__ARMCC_VERSION)
...
struct is_integral {
  static const bool value =
    is_same<T, signed char>::value || is_same<T, unsigned char>::value || is_same<T, signed short>::value || is_same<T, unsigned short>::value || is_same<T, signed int>::value || is_same<T, unsigned int>::value || is_same<T, signed long>::value || is_same<T, unsigned long>::value ||
...
public:
  ConstRamStringAdapter(const char *str = 0)
    : _str(str) {}
...
public:
  RamStringAdapter(const char *str)
    : ConstRamStringAdapter(str) {}
...
public:
  SizedRamStringAdapter(const char *str, size_t n)
    : _str(str), _size(n) {}
  int8_t compare(const char *other) const {
...
public:
  StlStringAdapter(const std::string &str)
    : _str(&str) {}
  char *save(MemoryPool *pool) const {
...
public:
  ArduinoStringAdapter(const ::String &str)
    : _str(&str) {}
  char *save(MemoryPool *pool) const {
...
public:
  FlashStringAdapter(const __FlashStringHelper *str)
    : _str(str) {}
  int8_t compare(const char *other) const {
...
template<typename TOut, typename TIn>
typename enable_if<is_integral<TOut>::value && is_signed<TOut>::value && sizeof(TOut) <= sizeof(TIn),
                   bool>::type
canStoreNegativeInteger(TIn value) {
  return value <= TIn(numeric_limits<TOut>::highest()) + 1;
}
template<typename TOut, typename TIn>
typename enable_if<is_integral<TOut>::value && is_signed<TOut>::value && sizeof(TIn) < sizeof(TOut),
                   bool>::type
canStoreNegativeInteger(TIn) {
  return true;
}
...
public:
  ...
  bool equals(const VariantData &other) const {
    ...
    switch (type()) {
      ...
      case VALUE_IS_LINKED_RAW:
      case VALUE_IS_OWNED_RAW:
        return _content.asRaw.size == other._content.asRaw.size && !memcmp(_content.asRaw.data, other._content.asRaw.data, _content.asRaw.size);
      case VALUE_IS_BOOLEAN:
      case VALUE_IS_POSITIVE_INTEGER:
      case VALUE_IS_NEGATIVE_INTEGER:
...
public:
  DynamicStringWriter(String &str)
    : _str(&str) {}
...
public:
  DynamicStringWriter(std::string &str)
    : _str(&str) {}
  size_t write(uint8_t c) {
    _str->operator+=(static_cast<char>(c));
    return 1;
...
template<typename T>
inline typename enable_if<is_same<T, const char *>::value || is_same<T, char *>::value,
                          const char *>::type
...
public:
  ...
  template<typename T>
  FORCE_INLINE typename enable_if<is_same<T, const char *>::value || is_same<T, char *>::value || IsWriteableString<T>::value,
                                  bool>::type
  is() const {
...
public:
  ...
  template<typename T>
  FORCE_INLINE bool set(
    T value, typename enable_if<is_integral<T>::value && is_unsigned<T>::value>::type * = 0) const {
  ...
  template<typename T>
  FORCE_INLINE typename enable_if<!is_same<T, ArrayRef>::value && !is_same<T, ObjectRef>::value && !is_same<T, VariantRef>::value,
...
public:
  VariantConstRef()
    : base_type(0) {}
  VariantConstRef(const VariantData *data)
    : base_type(data) {}
  VariantConstRef(VariantRef var)
    : base_type(var._data) {}
  template<typename Visitor>
  void accept(Visitor &visitor) const {
    variantAccept(_data, visitor);
...
class VariantPtr {
public:
  VariantPtr(MemoryPool *pool, VariantData *data)
    : _variant(pool, data) {}
  VariantRef *operator->() {
    return &_variant;
  }
...
public:
  VariantConstPtr(const VariantData *data)
    : _variant(data) {}
...
public:
  ArrayConstRefIterator()
    : _slot(0) {}
  explicit ArrayConstRefIterator(const VariantSlot *slot)
    : _slot(slot) {}
...
public:
  ...
  FORCE_INLINE ArrayConstRef()
    : base_type(0) {}
  FORCE_INLINE ArrayConstRef(const CollectionData *data)
    : base_type(data) {}
...
class PairPtr {
public:
  PairPtr(MemoryPool *pool, VariantSlot *slot)
    : _pair(pool, slot) {}
...
class PairConstPtr {
public:
  PairConstPtr(const VariantSlot *slot)
    : _pair(slot) {}
...
public:
  ObjectConstIterator()
    : _slot(0) {}
  explicit ObjectConstIterator(const VariantSlot *slot)
    : _slot(slot) {}
...
public:
  ...
  ObjectConstRef()
    : base_type(0) {}
  ObjectConstRef(const CollectionData *data)
    : base_type(data) {}
  FORCE_INLINE iterator begin() const {
    if (!_data) return iterator();
    return iterator(_data->head());
...
protected:
  AllocatorOwner() {}
  AllocatorOwner(const AllocatorOwner &src)
    : _allocator(src._allocator) {}
  AllocatorOwner(TAllocator allocator)
...
public:
  ...
  StaticJsonDocument(VariantRef src)
    : JsonDocument(_buffer, _capacity) {
    set(src);
  }
...
struct ParsedNumber {
  ParsedNumber()
    : uintValue(0), floatValue(0), _type(VALUE_IS_NULL) {}
  ParsedNumber(TUInt value, bool is_negative)
    : uintValue(value),
      floatValue(TFloat(value)),
      _type(uint8_t(is_negative ? VALUE_IS_NEGATIVE_INTEGER
                                : VALUE_IS_POSITIVE_INTEGER)) {}
  ParsedNumber(TFloat value)
    : floatValue(value), _type(VALUE_IS_FLOAT) {}
  template<typename T>
  T as() const {
    switch (_type) {
      case VALUE_IS_NEGATIVE_INTEGER:
...
public:
  class StringBuilder {
  public:
    StringBuilder(char **ptr)
      : _writePtr(ptr), _startPtr(*ptr) {}
    void append(char c) {
      *(*_writePtr)++ = char(c);
    }
    char *complete() const {
...
struct NestingLimit {
  NestingLimit()
    : value(ARDUINOJSON_DEFAULT_NESTING_LIMIT) {}
...
private:
  ...
  static inline bool canBeInNonQuotedString(char c) {
    return isBetween(c, '0', '9') || isBetween(c, '_', 'z') || isBetween(c, 'A', 'Z') || c == '+' || c == '-' || c == '.';
  }
...
public:
  PrettyJsonSerializer(TWriter &writer)
    : base(writer), _nesting(0) {}
...
public:
  MsgPackSerializer(TWriter &writer)
    : _writer(&writer), _bytesWritten(0) {}
  template<typename T>
